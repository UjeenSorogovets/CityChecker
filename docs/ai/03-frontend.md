...
SPA: `src/CityChecker.Api/wwwroot/` — no bundler. Entry: `index.html` loads `app.js?v=svgrotate3` as ES module. Map rotation: **leaflet-rotate** 0.2.8 (CDN).
...
- Vectors are **SVG in `rotatePane`** (same CSS rotate as tiles). Never `preferCanvas` with leaflet-rotate. Also **unbind Renderer `rotate`→`_update`** — that second transform is what made houses/GPS circle lag and jerk.  
...
| `footprintLayer` | OSM building polygons (Wołomin / Wrocław) |
